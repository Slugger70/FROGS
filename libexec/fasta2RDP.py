...
def treeFromFasta( in_fasta, ranks ):
    ...
    for record in FH_databank:
        desc = unicodedata.normalize('NFD', unicode(record.description,"utf-8")).encode('ascii', 'ignore')
        if desc.endswith(";"):
            desc=desc[:-1]
...
def mergeFastaTax( in_tax, in_fasta, out_fasta ):
    ...
    for line in FH_tax:
        ...
        tax = unicodedata.normalize('NFD', unicode(taxonomy,"utf-8")).encode('ascii', 'ignore')
        if tax.endswith(";"):
...
def writeRDPFasta(tree, in_fasta, out_fasta):
    ...
    for record in FH_in_db:
        ...
        for taxa in taxonomy:
            tax = unicodedata.normalize('NFD', unicode(taxa,"utf-8")).encode('ascii', 'ignore')
...
if __name__ == "__main__":
    ...
    # Write RDP tax
    FH_RDPTax = open(args.rdp_taxonomy, "w")
